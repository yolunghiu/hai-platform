...
set_env() {
  ...
  : ${SHARED_FS_ROOT:="/NAS/hai-platform"}
  ...
  : ${TRAINING_NODES:="cn-beijing.10.0.0.190"}
  # jupyter compute nodes list, format: "node1 node2", JUPYTER_NODES should differ from TRAINING_NODES
  : ${JUPYTER_NODES:="cn-beijing.10.0.0.191"}
  # service nodes which running task manager, format: "node1 node2"
  : ${MANAGER_NODES:="cn-beijing.10.0.0.186"}

  # all in one image
  : ${BASE_IMAGE:="registry.cn-hangzhou.aliyuncs.com/hfai/hai-platform:latest-202207"}
  # train image
...
parse_user_info() {
  ...
  # Trim trailing comma and any whitespace characters
  USER_INFO_DB=$(echo $USER_INFO_DB | sed 's/[,[:space:]]*$//')
...
parse_host_info() {
  ...
  # Trim trailing comma and any whitespace characters
  HOST_INFO_DB=$(echo $HOST_INFO_DB | sed 's/[,[:space:]]*$//')
...
parse_env() {
  ...
  # Trim trailing comma and any whitespace characters
  MANAGER_NODES_FORMATTED=$(echo $MANAGER_NODES_FORMATTED | sed 's/[,[:space:]]*$//')
